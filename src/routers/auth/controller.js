...
import { generateUsernames, traffic_from, user_types } from "../../utils";
...
export async function addUser(body) {
  try {
    ...
    let user_exist = await user_model.findOne({
...
export async function getUserName(username) {
  try {
    ...
    let all_users = await (
      await user_model.find({}, { _id: 0, USERNAME: 1 })
    ).map((ele) => ele.USERNAME);

    let filteredUsers = usernames.filter((ele) =>
      all_users.some((item) => item != ele)
    );
...
export async function getGoogleAuthenticatedUser(token) {
  try {
    ...
    let exist = await user_model.findOne({ EMAIL: objPrototype.EMAIL });
    if (exist == null) {
